...
export default function ArtistDetail() {
    ...
    const detail = useAppSelector((state: RootState) => state.artists.detail);
    const { id } = useParams();
    ...
    return (
        <div>
            ...
            {(detail && detail.length > 0) ? <>
                        ...
                        <div className="relative flex flex-row bg-gray-50 shadow-2xl rounded-lg w-4/5 p-10">
                            ...
                            <div className="flex flex-row items-center justify-center w-1/2">
                                <div className="w-full h-96">
                                    <img src={detail[0].img} className="object-contain w-full h-full rounded-full" alt="" />
                                </div>
                            </div>
                            <div className="w-1/2 flex flex-col justify-between">
                                <div className="mb-5">
                                    <p className="text-3xl font-extrabold">Nombre del artista</p>
                                    <p className="text-2xl font-light">{detail[0].name}</p>
                                </div>
                                <div className="mb-5">
                                ...
                                <div className="mb-5 flex flex-row items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-base font-light">{detail[0].usuario.persona.city} - {detail[0].usuario.persona.country}</p>
                                    </div>
                                    <div>
                ...
                <EventsArtist idArtist={detail[0].id} />
            </> : ''}
        </div>
    )
}